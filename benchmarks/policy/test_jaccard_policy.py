...
from generate_jaccard_policy_header import (
    casefold_entries,
    emit,
    grams,
    parse,
    scalar_lower,
    threshold_milli as policy_threshold_milli,
)
...
class JaccardKeywordTests(unittest.TestCase):
    ...
    def test_policy_threshold_requires_exact_thousandth_precision(self) -> None:
        cases = (("0.8", 800), ("0.691", 691), ("1.0", 1000), ("0", 0))
        for value, expected in cases:
            with self.subTest(value=value):
                self.assertEqual(policy_threshold_milli(value), expected)

        for value in (0.6914, 0.0005):
            with self.subTest(value=value):
                with self.assertRaisesRegex(ValueError, "thousandth precision"):
                    policy_threshold_milli(value)

    def test_policy_rejects_unrepresentable_threshold_instead_of_rounding(self) -> None:
        policy = {
            "routes": [{
                "name": "code_ngram",
                "route": "coding",
