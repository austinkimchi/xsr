...
MAX_QUERY_TOKENS = 131_072
MAX_FIXED_SCORE = (1 << 63) - 1
MAX_FIXED_TERM_WEIGHT = MAX_FIXED_SCORE // MAX_QUERY_TOKENS
...
def token_bytes(token: str) -> bytes:
    ...
    if len(encoded) > MAX_TOKEN_BYTES:
        raise ValueError(
            f"BM25 token {token!r} is {len(encoded)} bytes; maximum is {MAX_TOKEN_BYTES}"
        )
    return encoded


def threshold_fixed(value: object) -> int:
    result = int((Decimal(str(value)) * SCORE_SCALE).quantize(Decimal("1"), rounding=ROUND_HALF_UP))
    if not 0 <= result <= MAX_FIXED_SCORE:
        raise ValueError(
            f"bm25_threshold {value!r} exceeds the Q1e9 signed-64 accumulator bound"
        )
    return result
...
def parse(path: Path) -> tuple[list[dict[str, object]], list[dict[str, object]], list[dict[str, object]]]:
    ...
    for signal in signals:
        ...
        rule_threshold = threshold_fixed(signal.get("bm25_threshold", 0.1))
        ...
        for tokens in corpus:
            ...
            documents.append({"rule_id": len(rules), "threshold_fixed": rule_threshold})
            ...
            for token, tf in frequencies.items():
                ...
                weight = int(
                    (Decimal(str(idf * tf_norm)) * SCORE_SCALE).quantize(
                        Decimal("1"), rounding=ROUND_HALF_UP
                    )
                )
                if not 0 < weight <= MAX_FIXED_TERM_WEIGHT:
                    raise ValueError(
                        f"BM25 term weight for {token!r} exceeds the Q1e9 signed-64 "
                        f"accumulator bound for MAX_QUERY_TOKENS={MAX_QUERY_TOKENS}"
                    )
                vector = term_vectors.setdefault(hashed, [0] * MAX_DOCUMENTS)
                vector[doc_id] = weight

        rules.append({
            "threshold_fixed": rule_threshold,
    ...
    terms.extend(
        {
            "hash": key,
            "token": stopword_hashes[key],
            "weights": [0] * MAX_DOCUMENTS,
            "stopword": True,
        }
        for key in sorted(stopword_hashes)
    )
    for term in terms:
        token_bytes(str(term["token"]))
...
def emit(source: Path, rules: list[dict[str, object]], documents: list[dict[str, object]], terms: list[dict[str, object]]) -> str:
    thresholds = [f"{document['threshold_fixed']}LL" for document in documents]
    thresholds.extend("0LL" for _ in range(MAX_DOCUMENTS - len(documents)))
    lines = [
        "/* Generated by generate_bm25_policy_header.py. Do not edit. */",
        f"/* Source: {source.as_posix()}; bm25 crate 2.3 defaults, Q1e9 scores. */",
        ...
        f"  .thresholds_fixed = {{{', '.join(thresholds)}}},", "};",
        "static const struct xdp_bm25_rule xdp_bm25_generated_rules[] = {",
    ]
    for rule in rules:
        lines.append("  {.threshold_fixed = %(threshold_fixed)sLL, .priority = %(priority)s, .route = %(route)s, .operator = %(operator)s, .document_start = %(document_start)s, .document_count = %(document_count)s, .document_mask = %(document_mask)s}," % rule)
    ...
    for term in terms:
        encoded = token_bytes(str(term["token"]))
        lines.append(
            f"  {{.hash = {term['hash']}U, .value = {{.weights = "
            f"{{{', '.join(f'{weight}LL' for weight in term['weights'])}}}, "
            f".token = {{{', '.join(map(str, encoded))}}}, "
            f".token_length = {len(encoded)}, "
