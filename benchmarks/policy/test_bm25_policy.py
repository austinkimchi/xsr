...
class Bm25CompatibilityTests(unittest.TestCase):
    ...
    def test_score_just_below_threshold_routes_others(self) -> None:
        query = "python function"
        keywords = [
            "python function",
            "python alpha beta",
            "function gamma delta",
            "function epsilon zeta",
        ]
        threshold = 1.181661
        floating = keyword_scores(query, keywords)
        fixed = fixed_scores(query, keywords)
        fixed_threshold = threshold_fixed(threshold)

        self.assertAlmostEqual(floating[0], 1.1816602517613024, places=15)
        self.assertEqual(fixed[0], 1181660252)
        self.assertEqual(fixed_threshold, 1181661000)
        self.assertFalse(rule_matches(query, keywords, "OR", threshold))
        fixed_match = any(
            score > 0 and score >= fixed_threshold for score in fixed
        )
        self.assertEqual("coding" if fixed_match else "others", "others")
    ...
    def test_generator_and_bpf_share_the_token_bound(self) -> None:
        classifier = (
            ROOT / "bpf" / "stages" / "signals" / "xdp_bm25_classifier.bpf.h"
        ).read_text(encoding="utf-8")
        self.assertIn(
            f"#define XDP_BM25_MAX_TOKEN_BYTES {MAX_TOKEN_BYTES}", classifier
        )
        self.assertIn("__s64 thresholds_fixed", classifier)
        self.assertIn("__s64 weights", classifier)
        self.assertIn("__s64 scores", classifier)
        self.assertEqual(MAX_QUERY_TOKENS, (256 * 1024 + 1) // 2)
        self.assertLessEqual(
            MAX_FIXED_TERM_WEIGHT * MAX_QUERY_TOKENS, (1 << 63) - 1
        )
    ...
    def test_policy_rejects_tokens_beyond_runtime_comparison_bound(self) -> None:
        long_token = "a" * (MAX_TOKEN_BYTES + 1)
        source = (ROOT / "config" / "policy_bm25.yaml").read_text()
        source = source.replace('keywords: ["code",', f'keywords: ["{long_token}",', 1)
        with tempfile.TemporaryDirectory() as directory:
            policy = Path(directory) / "policy.yaml"
    ...
    def test_fnv_collision_requires_exact_generated_surface(self) -> None:
        configured = "code"
        collision = "zsydrxi"
        self.assertEqual(token_hash(configured), token_hash(collision))
        self.assertEqual(token_hash(configured), 4180765940)

        _, _, terms = parse(ROOT / "config" / "policy_bm25.yaml")
        by_hash = {int(term["hash"]): term for term in terms}

        def runtime_weights(token: str) -> list[int] | None:
            term = by_hash.get(token_hash(token))
            if term is None or term["token"] != token:
                return None
            return list(term["weights"])

        code_weights = runtime_weights(configured)
        self.assertIsNotNone(code_weights)
        self.assertTrue(any(code_weights or ()))
        self.assertIsNone(runtime_weights(collision))
        self.assertEqual(runtime_weights(collision), runtime_weights("unknown-token"))

    def test_generated_policy_terms_still_reject_fnv_collisions(self) -> None:
        source = (ROOT / "config" / "policy_bm25.yaml").read_text()
        source = source.replace(
            'keywords: ["code", "function",',
            'keywords: ["code", "zsydrxi",',
            1,
        )
        with tempfile.TemporaryDirectory() as directory:
            policy = Path(directory) / "policy.yaml"
            policy.write_text(source)
            with self.assertRaisesRegex(ValueError, "FNV-1a collision"):
                parse(policy)
    ...
    def test_policy_generates_vocabulary_aliases_and_stopwords(self) -> None:
        ...
        self.assertEqual(by_hash[token_hash("the")]["token"], "the")
    ...
    def test_threshold_rounding(self) -> None:
        self.assertEqual(threshold_fixed("0.1000005"), 100000500)
