#!/usr/bin/env python3
"""Preprocess a VSR n-gram keyword policy for the XSR Jaccard maps.

The output intentionally contains userspace-only initializers. The userspace
router loads them into BPF array maps, keeping preprocessing out of BPF.
"""
...
def threshold_milli(value: object) -> int:
    ...
    if not Decimal("0") <= threshold <= Decimal("1"):
        raise ValueError("ngram_threshold must be between 0 and 1")

    scaled = threshold * 1000
    if scaled != scaled.to_integral_value():
        raise ValueError(
            "ngram_threshold must be representable to thousandth precision"
        )
