...
# Historical experiments A-C use this default. Deployment serialization and
# inference use the independently frozen 8K constant below.
FEATURE_COUNT = 4096
DEPLOYMENT_FEATURE_COUNT = 8192
CLASS_COUNT = 14
...
def dense_counts(
    ...
    return np.bincount(
        feature_indices(prompt, limit, feature_count), minlength=feature_count
    )
...
def float_scores(prompt: str, weights: np.ndarray, bias: np.ndarray) -> np.ndarray:
    if weights.ndim != 2 or weights.shape[0] != CLASS_COUNT:
        raise ValueError("weights must have shape [14, hash buckets]")
    indices = feature_indices(prompt, feature_count=weights.shape[1])
...
def read_kernel_model(
    path: Path, required_feature_count: int | None = None,
) -> QuantizedModel:
    ...
    if (magic, version, classes, prompt_limit) != (
        MODEL_MAGIC, MODEL_VERSION, CLASS_COUNT, PROMPT_BYTE_LIMIT
    ) or features <= 0 or features & (features - 1):
        raise ValueError("incompatible kernel model header")
    if required_feature_count is not None and features != required_feature_count:
        raise ValueError(
            f"model has {features} buckets; required {required_feature_count}"
        )
    expected = header_size + CLASS_COUNT * 4 + features * CLASS_COUNT
...
def read_deployment_model(path: Path) -> QuantizedModel:
    """Read only the architecture accepted by the live XSR model loader."""
    return read_kernel_model(path, required_feature_count=DEPLOYMENT_FEATURE_COUNT)
