#!/usr/bin/env python3
"""Experiment C: train unchanged 4K students on expanded balanced data."""
...
def main() -> None:
    ...
    if any(item.label < 0 for split in examples.values() for item in split):
        raise ValueError("Experiment C requires ground-truth labels for every row")
    train_counts = Counter(item.label for item in examples["train"])
    if len(set(train_counts.values())) != 1 or len(train_counts) != CLASS_COUNT:
        raise ValueError(f"Experiment C training rows are not class-balanced: {train_counts}")
    ...
    def metrics(model: Student, rows: list[Example]) -> dict:
        ...
        with torch.inference_mode():
            for indices, offsets, labels, teacher_logits in batches(rows, list(range(len(rows)))):
    ...
    for name, distilled in (("supervised", False), ("distilled", True)):
        ...
        np.savez_compressed(args.output_dir / f"{name}_float.npz", weights=weights, bias=bias)
        models[name] = {
    ...
    report = {
        ...
        "training": {
            ...
            "examples_per_class": {
                LABELS[index]: train_counts[index] for index in range(CLASS_COUNT)
            },
            "epochs": args.epochs,
