...
class RouterArenaAdapterTests(unittest.TestCase):
    def test_unicode_input_categories(self) -> None:
        self.assertEqual(benchmark.unicode_input_category("plain ASCII"), "ascii")
        self.assertEqual(benchmark.unicode_input_category("hello—world"), "punctuation-symbol")
        self.assertEqual(benchmark.unicode_input_category("café"), "latin-diacritic")
        self.assertEqual(benchmark.unicode_input_category("cafe\N{COMBINING ACUTE ACCENT}"), "latin-diacritic")
        self.assertEqual(benchmark.unicode_input_category("why 为什么"), "non-latin")

    def test_input_support_uses_the_stream_request_byte_bound(self) -> None:
        self.assertIsNone(benchmark.input_support_reason("short prompt"))
        self.assertEqual(
            benchmark.input_support_reason("x" * benchmark.MAX_STREAM_REQUEST_BYTES),
            "stream-request-byte-bound",
        )
...
class RoutingCorrectnessComparisonTests(unittest.TestCase):
    ...
    def test_source_index_collisions_use_case_id_and_report_per_source(self) -> None:
        ...
        comparison = benchmark.xsr_vsr_routing_agreement(
            [mode_result("xsr", xsr_observations), mode_result("vllm-sr", list(xsr_observations))]
        )
    ...
    def test_route_permutation_is_not_treated_as_agreement(self) -> None:
        ...
        comparison = benchmark.xsr_vsr_routing_agreement([xsr, vllm])
        self.assertEqual(comparison["status"], "ok")
    ...
    def test_mixed_reference_kinds_and_routerarena_breakdown(self) -> None:
        ...
        summary = benchmark.summarize("xsr", results, 1.0, None, None)
    ...
    def test_mixed_report_keeps_reference_labels_separate(self) -> None:
        ...
        mode_results = [
            benchmark.summarize("xsr", results, 1.0, None, None),
