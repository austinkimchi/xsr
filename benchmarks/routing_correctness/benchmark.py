#!/usr/bin/env python3
"""Compare XSR and vLLM-SR on the shared keyword routing policy."""
...
DEFAULT_XSR_URL = "http://10.10.0.1:18081/v1/chat/completions"
    ...
    try:
        while len(rows) < SPEED_BENCH_ROWS:
            ...
            request = urllib.request.Request(
                url,
                headers={"User-Agent": "xsr-router-benchmark/1.0"},
            )
...
def summarize(
    ...
    return {
        ...
        # Keep each observation and its label match so XSR and vLLM-SR can be
...
def ensure_network_namespace(args: argparse.Namespace) -> str | None:
    if os.geteuid() != 0:
        return "network-namespace mode requires root"
    if not shutil.which("ip"):
        return "iproute2 is required"
    if args.setup:
        checked(["make", "setup"])
    for command in (
        ["ip", "link", "show", "dev", args.xsr_ifname],
        ["ip", "netns", "exec", args.xsr_netns, "true"],
    ):
        try:
            checked(command)
    ...
    if not args.no_build:
        # The benchmark reference matcher and XSR maps must be generated from
        # the exact same policy file, including when --config is custom.
...
def sanitize_benchmark_processes() -> None:
    ...
    process_names = ("sk_router", "mock_backend")
...
def run_direct_netns(args: argparse.Namespace, cases: list[Case]) -> dict[str, Any]:
    reason = ensure_network_namespace(args)
    if reason:
    ...
    try:
        if not args.no_firewall:
            iptables_allow(args.xsr_ifname, args.xsr_backend_port)
            firewall = True
        ...
        if firewall:
            iptables_remove(args.xsr_ifname, args.xsr_backend_port)


def run_sockmap(args: argparse.Namespace, cases: list[Case]) -> dict[str, Any]:
    if os.geteuid() != 0:
        return {"mode": "xsr", "execution_mode": "sockmap", "status": "skipped", "reason": "SOCKMAP mode requires root"}
    if not shutil.which("ip"):
        return {"mode": "xsr", "execution_mode": "sockmap", "status": "skipped", "reason": "iproute2 is required"}
    if args.setup:
        checked(["make", "setup"])
    try:
        checked(["ip", "netns", "exec", args.xsr_netns, "true"])
    except subprocess.CalledProcessError:
        return {"mode": "xsr", "execution_mode": "sockmap", "status": "skipped", "reason": "missing prerequisite: network namespace"}
    if not args.no_build:
        checked(["make", f"KEYWORD_POLICY={args.config.resolve()}", "dev"])
    ...
    try:
        ...
        if not wait_for_router(router, out, "SK_SKB router listening", startup_logs):
            ...
            return {"mode": "xsr", "execution_mode": "sockmap", "status": "skipped", "reason": reason}

        cpu_start = read_cpu()
        start = time.perf_counter()
        results = netns_requests(args, cases)
        wall_s = time.perf_counter() - start
        summary = summarize("xsr", results, wall_s, cpu_delta(cpu_start, read_cpu()), sampled_cpu(pid=router.pid), len(cases))
...
def comparison(results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if "xsr" not in ok or "vllm-sr" not in ok:
        return {"status": "incomplete", "reason": "both modes must complete"}
    xsr, vllm = ok["xsr"], ok["vllm-sr"]
    xsr_avg = xsr["latency_ms"]["avg"]
    vllm_avg = vllm["latency_ms"]["avg"]
    xsr_p99 = xsr["latency_ms"]["p99"]
    vllm_p99 = vllm["latency_ms"]["p99"]
    xsr_rps = xsr["requests_per_second"]
    vllm_rps = vllm["requests_per_second"]

    avg_speedup = (vllm_avg / xsr_avg) if (vllm_avg and xsr_avg) else None
    p99_speedup = (vllm_p99 / xsr_p99) if (vllm_p99 and xsr_p99) else None
    rps_speedup = (xsr_rps / vllm_rps) if (xsr_rps and vllm_rps) else None

    return {
        "status": "ok",
        "route_agreement_delta": (
            xsr["route_agreement"] - vllm["route_agreement"]
            if xsr.get("route_agreement") is not None and vllm.get("route_agreement") is not None
            else None
        ),
        "avg_latency_delta_ms": (xsr_avg or 0) - (vllm_avg or 0),
        "p99_latency_delta_ms": (xsr_p99 or 0) - (vllm_p99 or 0),
...
def xsr_vsr_routing_agreement(results: list[dict[str, Any]]) -> dict[str, Any]:
    """Compare XSR and vLLM-SR routes for the exact same prompts.
    ...
    if not xsr or not vsr:
        return {"status": "incomplete", "reason": "both xsr and vllm-sr modes must be run"}
    if xsr.get("status") != "ok" or vsr.get("status") != "ok":
        return {
            "status": "incomplete",
            "reason": "both xsr and vllm-sr modes must complete",
    ...
    xsr_by_prompt, error = index_mode(xsr, "xsr")
    if error:
        ...
        request_comparisons.append(
            {
                ...
                "xsr_reference_match": xsr_result.get("reference_route_match"),
...
def write_reports(
    ...
    if pairwise["status"] == "ok":
        ...
        if pairwise["mismatches"]:
            lines += ["", "### Mismatches", "", "| Case ID | Expected | XSR | VSR | Prompt |", "| --- | --- | --- | --- | --- |"]
            for item in pairwise["mismatches"][:20]:
                prompt = " ".join(item["prompt"].split())[:160].replace("|", "\\|")
                lines.append(
                    f"| `{item['case_id']}` | {item['expected_route']} | {item['xsr_route']} | {item['vllm_sr_route']} | {prompt} |"
                )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--xsr-url", default=DEFAULT_XSR_URL)
    parser.add_argument("--xsr-ifname", default="veth0")
    parser.add_argument("--xsr-netns", default="ns1")
    parser.add_argument("--xsr-backend-port", type=int, default=18081)
...
def main() -> int:
    ...
    if {"direct-netns", "sockmap"}.intersection(modes):
        ...
        with contextlib.ExitStack() as stack:
            if not args.no_mock_backends:
                if mode == "direct-netns":
                    stack.enter_context(mock_backend(args.xsr_backend_port, "others"))
