...
def config(gateway: str, ports: dict[str, int], port: int, fixed_backend: str | None = None) -> dict[str, object]:
    selected_routes = (fixed_backend,) if fixed_backend else ROUTES
    routes = [] if fixed_backend else [
        {
    ...
    routes.append({"match": {"prefix": "/"}, "route": {"cluster": f"{fixed_backend or 'coding'}_cluster", "timeout": "30s"}})
    clusters = [
        {
...
def main() -> None:
    ...
    parser.add_argument("--fixed-backend", choices=ROUTES)
    args = parser.parse_args()
    ports = {route: getattr(args, f"{route}_port") for route in ROUTES}
    args.output.write_text(json.dumps(config(args.gateway, ports, args.port, args.fixed_backend), indent=2) + "\n", encoding="utf-8")
