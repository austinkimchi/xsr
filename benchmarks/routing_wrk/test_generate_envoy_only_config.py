...
class EnvoyOnlyConfigTest(unittest.TestCase):
    ...
    def test_forwarding_ablation_has_one_fixed_backend(self) -> None:
        rendered = config("172.18.0.1", {route: 18000 + index for index, route in enumerate(ROUTES)}, 8898, "coding")
        self.assertEqual(len(rendered["static_resources"]["clusters"]), 1)
        text = str(rendered)
        self.assertNotIn("ext_proc", text)
        self.assertNotIn("math_cluster", text)
        self.assertNotIn("x-benchmark-backend", text)
