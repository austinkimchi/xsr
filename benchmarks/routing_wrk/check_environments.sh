...
"$BENCHMARK_PYTHON" -c 'import aiohttp, datasets, numpy' \
...
if [ -n "${selected[envoy-only]+x}" ] && ! docker inspect "$VLLM_HOST" >/dev/null 2>&1; then
    envoy_image="${ENVOY_ONLY_IMAGE:-envoyproxy/envoy:v1.34-latest}"
    docker image inspect "$envoy_image" >/dev/null 2>&1 || \
        fail "Envoy-only requires either '$VLLM_HOST' or local image '$envoy_image'."
fi
