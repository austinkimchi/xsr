...
def statistics_for(values: list[float]) -> dict[str, float | None]:
    ...
    return {
        ...
        "ci95": (2.776 if len(values) == 5 else 1.96) * stdev / math.sqrt(len(values))
...
def fixed_rate_record(path: Path, raw_root: Path) -> dict[str, Any]:
    summary = json.loads(path.read_text(encoding="utf-8"))
    relative = path.relative_to(raw_root)
    if len(relative.parts) < 5:
        raise ValueError(f"unexpected fixed-rate summary path: {relative}")
    mode, configuration, trial_dir, system = relative.parts[:4]
    if mode != "fixed-rate" or not trial_dir.startswith("trial-"):
        raise ValueError(f"unexpected fixed-rate summary path: {relative}")
    error_count = sum(
        int(summary[name])
        for name in ("http_errors", "connection_errors", "timeout_errors", "other_errors")
    )
    interval_us = 1_000_000 / float(summary["requested_rps"])
    reasons: list[str] = []
    if error_count:
        reasons.append(f"request errors={error_count}")
    if summary["completed_requests"] != summary["scheduled_requests"]:
        reasons.append("incomplete requests")
    if summary["requests_scheduled_while_worker_busy"]:
        reasons.append("client worker backlog")
    if float(summary["max_absolute_scheduling_error_us"]) > interval_us:
        reasons.append("dispatch delay exceeded one target interval")
    topology = "docker-bridge" if system in {"envoy-only", "vsr", "extproc-student"} else "host-veth"
    latency = summary["actual_latency_us"]
    return {
        "mode": mode,
        ...
        "tool": "python fixed-rate client",
        "raw_output": str(path.with_name("requests.csv")),
        "valid": not reasons,
        "failure_reasons": reasons,
        "metrics": {
            "throughput_rps": float(summary["achieved_rps"]),
            "average_latency_us": float(latency["mean"]),
            "p50_latency_us": float(latency["p50"]),
...
def load_records(run_dir: Path) -> list[dict[str, Any]]:
    ...
    records = [
        ...
        for path in sorted(raw_root.glob("**/result.json"))
    ]
    records.extend(
        fixed_rate_record(path, raw_root)
        for path in sorted(raw_root.glob("fixed-rate/**/summary.json"))
    )
    return records
...
def main() -> None:
    ...
    if not records:
        raise SystemExit(f"no benchmark result artifacts under {args.run_dir / 'raw'}")
