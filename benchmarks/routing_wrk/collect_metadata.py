...
def xsr_warmup_note(lifecycle: str) -> str:
    if lifecycle == "same-process-persistent-connection-warmup":
        return (
            "Warm-up and measurement use the same router process and the same "
...
def main() -> None:
    ...
    metadata: dict[str, Any] = {
        ...
        "benchmark": {"mode": args.mode, "profile": args.profile, "trial_count": args.trials, "duration": args.duration,
                      ...
                      "load_generator": ({
                          "name": "python fixed-rate client",
                          "path": args.wrk_bin,
                          "sha256": sha256(Path(args.wrk_bin)),
                      } if args.mode == "fixed-rate" else {
                          "name": "wrk",
                          "path": args.wrk_bin,
                          "version": command(args.wrk_bin, "--version"),
                      })},
        "environment": {"virtualization": command("systemd-detect-virt"),
