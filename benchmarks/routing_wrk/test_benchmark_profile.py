...
class BenchmarkProfileTest(unittest.TestCase):
    def test_clean_capable_builds_precede_executable_mock_backend_check(self) -> None:
        source = SCRIPT.read_text(encoding="utf-8")
        prod_build = source.index('SK_PROFILE_COMPONENTS="$XSR_COMPONENT_PROFILE" prod')
        dev_build = source.index('SK_PROFILE_COMPONENTS="$XSR_COMPONENT_PROFILE" dev')
        mock_build = source.index("make benchmarks/mock_backend")
        executable_check = source.index("if [ ! -x ./benchmarks/mock_backend ]; then")
        backend_start = source.index("# Start marker backends for routing.")

        self.assertLess(prod_build, mock_build)
        self.assertLess(dev_build, mock_build)
        self.assertLess(mock_build, executable_check)
        self.assertLess(executable_check, backend_start)
    ...
    def test_xsr_builds_generate_and_validate_the_requested_profile(self) -> None:
        source = SCRIPT.read_text(encoding="utf-8")
        self.assertGreaterEqual(source.count("if system_selected xsr; then"), 4)
    ...
    def test_paper_fixed_rate_dry_run_uses_python_client(self) -> None:
        output = dry_run(
            BENCHMARK_PROFILE="paper", BENCHMARK_MODE="fixed-rate",
        )
        self.assertIn("mode=fixed-rate", output)
        self.assertIn("rates=100", output)
        self.assertIn("random_seed=20260826", output)
        self.assertIn("concurrencies=4", output)
        self.assertIn("python_latency/client.py", output)
        self.assertIn("xsr_warmup_lifecycle=same-process-persistent-connection-warmup", output)
