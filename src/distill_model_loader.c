...
struct distill_bucket { int8_t weight[XSR_DISTILL_CLASSES]; };
struct distill_config {
  int32_t bias[XSR_DISTILL_CLASSES];
...
int populate_distill_model(struct bpf_object *obj, const char *path) {
  ...
  if (fread(&header, sizeof(header), 1, input) != 1 ||
      memcmp(header.magic, XSR_DISTILL_MODEL_MAGIC, 8) != 0 ||
      le32toh(header.version) != XSR_DISTILL_MODEL_VERSION ||
      le32toh(header.classes) != XSR_DISTILL_CLASSES ||
      le32toh(header.buckets) != XSR_DISTILL_BUCKETS ||
      le32toh(header.prompt_bytes) != XSR_DISTILL_PROMPT_BYTES ||
      le32toh(header.score_bound) > INT32_MAX ||
      fread(config.bias, sizeof(config.bias), 1, input) != 1) {
    fclose(input); errno = EINVAL; return -1;
  }
  for (uint32_t index = 0; index < XSR_DISTILL_CLASSES; index++)
  ...
  if (max_bias + (XSR_DISTILL_PROMPT_BYTES - 2) * 127 !=
      le32toh(header.score_bound)) {
    fclose(input); errno = EINVAL; return -1;
  }
  for (uint32_t key = 0; key < XSR_DISTILL_BUCKETS; key++)
