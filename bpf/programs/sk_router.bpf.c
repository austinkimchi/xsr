...
struct sk_http_flow_state {
  ...
  /* Execution time is accumulated across parser callbacks. It deliberately
   * excludes wall-clock time while the stream parser is not running. */
  __u64 profile_parse_signal_ns;
  __u64 profile_decision_ns;
#endif
};
...
#ifdef SK_PROFILE_COMPONENTS
/* One per-CPU aggregate per stage. All three entries are updated only after a
 * completed client request is successfully redirected to its backend. */
struct {
  __uint(type, BPF_MAP_TYPE_PERCPU_ARRAY);
  __uint(max_entries, SK_PROFILE_STAGE_COUNT);
  __type(key, __u32);
  __type(value, struct sk_profile_value);
} sk_profile_components SEC(".maps");

static __always_inline void sk_profile_record(__u32 stage, __u64 elapsed_ns) {
  struct sk_profile_value *value =
      bpf_map_lookup_elem(&sk_profile_components, &stage);
...
sk_profile_accumulate_parse(struct sk_http_flow_state *flow, __u64 start_ns) {
  flow->profile_parse_signal_ns += bpf_ktime_get_ns() - start_ns;
}
#endif
...
int sk_router_parser(struct __sk_buff *skb) {
  ...
  /* Ablation build: deliberately skip HTTP parsing and every signal stage.
   * Returning the bytes currently available makes the verdict program a
   * fixed SOCKMAP forwarding datapath. */
  return skb->len;
#else
  ...
  if (route_entry && (route_entry->flags & SK_ROUTER_FLAG_BACKEND))
    return skb->len;
#ifdef SK_PROFILE_COMPONENTS
  profile_start_ns = bpf_ktime_get_ns();
#endif
...
#ifdef SK_PROFILE_COMPONENTS
    sk_profile_accumulate_parse(flow, profile_start_ns);
#endif
    return 0;
  }
...
#ifdef SK_PROFILE_COMPONENTS
  flow->profile_decision_ns += bpf_ktime_get_ns() - profile_start_ns;
#endif
...
int sk_router_verdict(struct __sk_buff *skb) {
  ...
  int result;
#ifdef SK_PROFILE_COMPONENTS
  struct sk_http_flow_state *profile_flow;
  __u64 parse_signal_ns = 0;
  __u64 decision_ns = 0;
  __u64 redirect_ns;
  __u64 profile_start_ns;
  __u8 profile_ready = 0;
#endif
...
#ifdef XSR_FORWARDING_ONLY
  /* Predetermined backend: no signal generation or policy evaluation. */
  route = SK_ROUTE_CODING;
  model_id = SK_MODEL_CODING;
#else
#ifdef SK_PROFILE_COMPONENTS
  profile_flow = bpf_map_lookup_elem(&sk_http_flows, &cookie);
  if (profile_flow) {
    parse_signal_ns = profile_flow->profile_parse_signal_ns;
    decision_ns = profile_flow->profile_decision_ns;
    profile_ready = 1;
  }
  profile_start_ns = bpf_ktime_get_ns();
#endif
...
#ifdef SK_PROFILE_COMPONENTS
  decision_ns += bpf_ktime_get_ns() - profile_start_ns;
  profile_start_ns = bpf_ktime_get_ns();
#endif
  result =
      bpf_sk_redirect_map(skb, &sk_sock_map, target_slot, SK_REDIRECT_FLAGS);
#ifdef SK_PROFILE_COMPONENTS
  redirect_ns = bpf_ktime_get_ns() - profile_start_ns;
#endif
  if (result != SK_PASS) {
    increment_counter(COUNT_NO_PAYLOAD);
    struct sk_lifecycle_state *lifecycle =
...
#ifdef SK_PROFILE_COMPONENTS
    /* REDIRECT is the SK_SKB helper/verdict interval, not network delivery. */
    if (profile_ready) {
      sk_profile_record(SK_PROFILE_PARSE_SIGNAL, parse_signal_ns);
      sk_profile_record(SK_PROFILE_DECISION, decision_ns);
      sk_profile_record(SK_PROFILE_REDIRECT, redirect_ns);
    }
#endif
  }
  return result;
